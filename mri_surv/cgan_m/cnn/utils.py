...
def plot_time_auc(values, fname):
...
def w_estimator(obss, hits, time):
    # kaplan_meier_estimator?
    estimator = nonparametric.CensoringDistributionEstimator()
    y = np.array([(x, y) for x,y in zip(hits, obss)], dtype=[('hit','bool'),('time', 'int')])
    estimator.fit(y)
    y_inp = np.array([(1, time)], dtype=[('hit','bool'),('time', 'int')])
    out = estimator.predict_ipcw(y_inp)
    return out

def plot_roc(values, fname):
    hits_all, preds_all = values
    print(fname)
    print(classification_report(hits_all, preds_all))
    fpr, tpr, _ = roc_curve(hits_all, preds_all)
    auc_val = auc(fpr, tpr)
    plt.figure()
    plt.plot(
        fpr,
    ...
    plt.legend(loc="lower right")
    plt.savefig(fname+'roc_plot.png')
