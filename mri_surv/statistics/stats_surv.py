...
for i, mod in enumerate(models):
    ...
    for idx in range(0, 5):  # for each fold
        ...
        max_for_bs_adni = max(truth_adni.observed)
        max_for_bs_nacc = max(truth_nacc.observed)

        colnames = preds_adni.columns
        colnames = [x for x in colnames if 'pred.prob' in x]
        #bins = [int(re.findall(r'\d+', x)[0]) for x in colnames]
        bs_bins_adni = bins
        bs_bins_nacc = bins

        # print(preds_only_adni.head())

        preds_only_adni_bs = preds_only_adni
        preds_only_nacc_bs = preds_only_nacc
        preds_only_adni_bs = preds_only_adni[[
            'pred.prob.{}'.format(time) for time in bs_bins_adni]]
        preds_only_nacc_bs = preds_only_nacc[[
            'pred.prob.{}'.format(time) for time in bs_bins_nacc]]

        # correct max time bin
        # if max_for_bs_adni < bins[-1] or max_for_bs_nacc < bins[-1]:
        #     bs_bins_adni = [b for b in bins if b <= max_for_bs_adni]
        #     bs_bins_nacc = [b for b in bins if b <= max_for_bs_nacc]
        ...
        # print(bs_bins_nacc)
        # print(train_struc)
        # print(test_struc_nacc)

        # print(preds_only_adni_bs.head())
        ...
        ci_adni = np.mean(np.array([concordance_index_censored(truth_adni.hit.astype(
            bool), truth_adni.observed, 1-preds_only_adni['pred.prob.{}'.format(time)])[0] for time in bins[1:]]))

        # print(
        #    "Mean CI: {:.3f} +/- {:.3f}".format(np.mean(ci_adni), np.std(ci_adni)))

        preds_only_nacc_bs = preds_only_nacc_bs.mask(preds_only_nacc_bs < 0, 0)
        # print("NACC")
        bs_nacc = retrieve_brier_scores(
            bins, preds_only_nacc_bs, train_struc, test_struc_nacc)[0]

        ci_nacc = np.mean(np.array([concordance_index_censored(truth_nacc.hit.astype(
            bool), truth_nacc.observed, 1-preds_only_nacc['pred.prob.{}'.format(time)])[0] for time in bins[1:]]))

        print(ci_adni)

        # print("Mean CI: {:.3f}".format(ci_nacc)))

        # for time in bins:
        #     print(concordance_index_censored(truth_adni.hit.astype(bool),
        #                                      truth_adni.observed, 1 - preds_only_adni['pred.prob.{}'.format(time)]))
...
print(statspkg.f_oneway(
    ci_adni_df['mlp'], ci_adni_df['cnn'], ci_adni_df['cph'], ci_adni_df['weibull']))

print(statspkg.f_oneway(
    bs_adni_df['mlp'], bs_adni_df['cnn'], bs_adni_df['cph'], bs_adni_df['weibull']))

print(statspkg.f_oneway(
    ci_nacc_df['mlp'], ci_nacc_df['cnn'], ci_nacc_df['cph'], ci_nacc_df['weibull']))

print(statspkg.f_oneway(
    bs_nacc_df['mlp'], bs_nacc_df['cnn'], bs_nacc_df['cph'], bs_nacc_df['weibull']))
